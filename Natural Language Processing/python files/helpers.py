...
def prepare_train_dataset_qa(examples, tokenizer, max_seq_length=None):
    ...
    if type(examples['context'][0]) == str:
        contexts = examples["context"]
        #print(f"\nList Type: list of length {len(examples['context'])}")
    elif type(examples['context'][0]) == dict:
        examples['context_cleaned'] = ["".join([sentence[0] for sentence in sents['sentences']]) for sents in examples["context"]]
        contexts = examples['context_cleaned']
        examples['answers'] = [{'text': [example[0][:max_seq_length]], 'answer_start': [max(example[1].find(example[0]), 0)]} for example in zip(examples['answer'], examples['context_cleaned'])]
        #print(f"\nDict Type")
    else:
        print(f"New Type: {type(examples['context'])}")
    ...
    # print(i)
    # print(f"\nTokenized Examples Keys: {tokenized_examples.keys()}")
    # for key, value in tokenized_examples.items():
    #     print(f'Key: {key}, Type Value: {type(value)}, Length Value: {len(value)}')
    # for column in ['context_cleaned','answers']:
    #     examples.pop("nonexistent_key", None)
    return tokenized_examples
