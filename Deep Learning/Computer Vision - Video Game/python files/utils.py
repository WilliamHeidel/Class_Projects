...
class DetectionSuperTuxDataset(Dataset):
    def __init__(self, dataset_path, transform=dense_transforms.ToTensor(), min_size=20):
        ...
        from os import path
        self.files = []
        for im_f in glob(path.join(dataset_path, '*_labels.jpg')):
            self.files.append(im_f.replace('_labels.jpg', ''))
    ...
    def __getitem__(self, idx):
        import numpy as np
        b = self.files[idx]
        im = Image.open(b + '_labels.jpg')
        nfo = np.load(b + '_boxes.npz')
        data = im, self._filter(nfo['puck'])
...
if __name__ == '__main__':
    ...
    for i, ax in enumerate(axs.flat):
        ...
        ax.imshow(np.stack((r, alpha), axis=2), interpolation=None)
        ax.axis('off')
    fig.tight_layout()
